...
type CartAction =
  | { type: 'ADD_ITEM'; payload: { product: Product; quantity?: number; size?: string; color?: string; displayImage?: string } }
...
const CartContext = createContext<{
  ...
  addItem: (product: Product, quantity?: number, size?: string, color?: string, displayImage?: string) => void
...
function cartReducer(state: CartState, action: CartAction): CartState {
  switch (action.type) {
    case 'ADD_ITEM': {
      const { product, quantity = 1, size, color, displayImage } = action.payload
      ...
      if (existingIndex >= 0) {
        ...
        newItems = state.items.map((item, i) =>
          i === existingIndex ? { ...item, quantity: newQty, displayImage: item.displayImage || displayImage } : item
        )
      } else {
        const cappedQty = Math.min(quantity, product.stock)
        newItems = [...state.items, { product, quantity: cappedQty, size, color, displayImage }]
      }
    ...
    case 'REFRESH_STOCK': {
      ...
      // Preserve the cart item's selected display image and any color/size metadata.
      ...
      const newItems = state.items
        .map((item) => {
          ...
          const mergedProduct = {
            ...
            colorImages: fresh.colorImages ?? item.product.colorImages ?? null,
          }

          return { ...item, product: mergedProduct, quantity: cappedQty }
        })
        .filter((item) => item.quantity > 0)
      return { ...state, items: newItems, ...computeTotals(newItems) }
    }
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const addItem = (product: Product, quantity = 1, size?: string, color?: string, displayImage?: string) =>
    dispatch({ type: 'ADD_ITEM', payload: { product, quantity, size, color, displayImage } })
