...
export default function AdminProductsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const res = await fetch('/api/mgmt-heron/products', { credentials: 'include' })
  ...
  const handleDelete = async (product: Product) => {
    ...
    try {
      const res = await fetch(`/api/mgmt-heron/products?id=${product.id}`, {
        method: 'DELETE',
        credentials: 'include',
      })
      if (res.status === 401) { window.location.href = '/mgmt-heron/login'; return }
  ...
  const filtered = products.filter((p) => {
    const matchSearch = !search ||
      ...
      (p.admin_source_tag ?? '').toLowerCase().includes(search.toLowerCase())
    const matchCat = !categoryFilter || p.category === categoryFilter
    const matchSupplier = !supplierFilter || p.admin_source_tag === supplierFilter
    return matchSearch && matchCat && matchSupplier
  })

  const categories = Array.from(new Set(products.map((p) => p.category)))
  const suppliers = Array.from(new Set(products.map((p) => p.admin_source_tag).filter(Boolean))) as string[]
      ...
      <div style={{ flex: 1, padding: 32, overflowX: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 32 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <Link href="/mgmt-heron/products/new" className="btn-primary" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {suppliers.length > 0 && (
            <select value={supplierFilter} onChange={(e) => setSupplierFilter(e.target.value)}
              className="input-dark" style={{ minWidth: 160, paddingTop: 10, paddingBottom: 10 }}>
              <option value="" style={{ background: '#0A0A0F' }}>All Suppliers</option>
              {suppliers.map((s) => <option key={s} value={s} style={{ background: '#0A0A0F' }}>{s}</option>)}
            </select>
          )}
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <Link href="/mgmt-heron/products/new" className="btn-outline" style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)' }}>
                  {['Product', 'Category', 'Source', 'Price', 'Stock', 'Status', 'Actions'].map((h) => (
              ...
              <tbody>
                {filtered.map((p) => (
                  <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.15s' }}
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      {p.admin_source_tag
                        ? <span style={{ background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.2)', padding: '3px 10px', color: '#C9A84C', fontSize: '0.7rem', whiteSpace: 'nowrap' }}>{p.admin_source_tag}</span>
                        : <span style={{ color: 'rgba(245,242,236,0.15)', fontSize: '0.7rem' }}>—</span>
                      }
                    </td>
                    <td style={{ padding: '12px 16px' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', gap: 8 }}>
                        <Link href={`/mgmt-heron/products/edit?id=${p.id}`}
