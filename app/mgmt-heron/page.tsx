      ...
      <div style={{ flex: 1, padding: 32, overflowX: 'auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
            {outOfStock > 0 && (
              <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Link href="/mgmt-heron/products" style={{ color: '#f87171', fontSize: '0.75rem', textDecoration: 'underline' }}>View →</Link>
              </div>
            )}
            {lowStock > 0 && (
              <div style={{ background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.2)', padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Link href="/mgmt-heron/products" style={{ color: '#fbbf24', fontSize: '0.75rem', textDecoration: 'underline' }}>View →</Link>
              </div>
            )}
          </div>
        )}
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <Link href="/mgmt-heron/products" style={{ color: '#C9A84C', fontSize: '0.75rem', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', overflow: 'hidden' }}>
              {products.slice(0, 6).map((p, i) => (
                <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', borderBottom: i < 5 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                  ...
                  <Link href={`/mgmt-heron/products/edit?id=${p.id}`} style={{ color: 'rgba(245,242,236,0.3)', fontSize: '0.75rem', textDecoration: 'none', padding: '6px 12px', border: '1px solid rgba(255,255,255,0.08)', transition: 'all 0.15s', flexShrink: 0 }}
