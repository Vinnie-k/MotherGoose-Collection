...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const updates: Partial<Product> = {
      ...
      subcategory: body.subcategory ? String(body.subcategory) : null,
      // Use !== undefined so empty arrays ([]) are saved correctly (clearing colors/tags/images)
      ...(body.images !== undefined && { images: Array.isArray(body.images) ? body.images : [] }),
      ...(body.stock !== undefined && { stock: safeNum(body.stock) }),
      ...(body.tags !== undefined && { tags: Array.isArray(body.tags) ? body.tags : body.tags.split(',').map((t: string) => t.trim()).filter(Boolean) }),
      ...(body.colors !== undefined && { colors: Array.isArray(body.colors) ? body.colors : body.colors.split(',').map((c: string) => c.trim()).filter(Boolean) }),
      colorImages: body.colorImages ?? null,
      admin_source_tag: body.admin_source_tag ? String(body.admin_source_tag).trim() : null,
      featured: Boolean(body.featured),
      new_arrival: Boolean(body.new_arrival),
      gender: body.gender ?? null,
      sizes: Array.isArray(body.sizes) ? body.sizes : null,
      // Preserve rating and review_count so they aren't lost on edit
      ...(body.rating !== undefined && { rating: Number(body.rating) || 0 }),
      ...(body.review_count !== undefined && { review_count: Number(body.review_count) || 0 }),
      updated_at: new Date().toISOString(),
    }
